                    ...
                    <div class="row g-4">
                        ...
                        <div class="col-lg-12">
                            <div class="h-100 rounded">
                                <form @submit.prevent="editCategorySubmission" class="">
                               
                                    <input type="text" 
                                        class="w-100 form-control border-0 py-3 mb-4" placeholder="Enter Product Name" 
                                        autocomplete="off" 
                                       
                                        v-model="editCategory.editCategoryName" 
                                        name="user_name" 
                                        @input="editCategoryInputFieldValidation('name')"
                                    >
                                    <p v-if="errors.editCategory.editCategoryNameError" class="error text-danger">{{ errors.editCategory.editCategoryNameError }}</p>
                                    
                                    <button 
                                        class="w-100 btn form-control border-secondary py-3 bg-white text-primary " 
                                        type="submit"
                                    >
                                        <i v-if="editCategory.buttonLoaderFlag"class="fa bi bi-hourglass-top me-2 text-white"></i>
                                        <p v-else>Submit</p>
                                    </button>
                                </form>
                            </div>
